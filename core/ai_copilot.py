"""
core/ai_copilot.py – Interactive AI System Copilot & Doctor Engine
==================================================================
...
def resolve_copilot_provider(config_manager: Optional[Any] = None) -> str:
    ...
    raw = ""
    try:
        raw = str(config_manager.get("ai_copilot_provider", "") or "").strip()
    except Exception:
        raw = ""
    if raw:
        return canonicalize_copilot_provider(raw)
    try:
        if bool(config_manager.get("ai_copilot_cloud_enabled", False)):
            return "gemini"
    except Exception:
        pass
    return "auto"
...
class AICopilotEngine:
    ...
    def _attach_companion_postscript(self, reply_text: str, user_text: str) -> str:
        """Cite this machine's profile, goal, and diary when Gemini is unavailable."""
        try:
            from core.companion import local_grounding_text, should_emit_postscript
            extra = local_grounding_text(
                user_text,
                config_manager=self.config_manager,
            )
        except Exception:
        ...
        if not extra or extra in (reply_text or ""):
            return reply_text
        try:
