...
assert "Đang học" in copilot_widget.lbl_companion_badge.toolTip() or "Mới gặp" in copilot_widget.lbl_companion_badge.toolTip()
...
assert win.companion_card.btn_reflect.text() == "Phản tỉnh / Viết sổ tay"
assert hasattr(win.companion_card, "btn_manage")
assert "Ollama" not in win.companion_card.chk_reflect.text()
from core.companion import compute_stage as _cs, memory_answer as _mem, empty_states_vi as _empty
from ui.companion_card import CompanionDialog as _CompanionDialog
assert _cs(0) == 0 and _cs(1) == 1 and _cs(7) == 2 and _cs(21) == 3
_empty_copy = _empty()
assert "Chưa có nhật ký" in _empty_copy["diary"]
assert "vài ngày" in _empty_copy["diary"]
...
_comp_dlg = _CompanionDialog(config_manager=win.config_manager, parent=win)
assert hasattr(_comp_dlg, "list_diary") and hasattr(_comp_dlg, "btn_clear_all")
assert _comp_dlg.btn_reflect.text() == "Phản tỉnh / Viết sổ tay"
_comp_dlg.close()
